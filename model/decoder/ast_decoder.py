...
from model.decoder.decoder_cell import Astormer, LSTM, ONLSTM
...
class ASTDecoder(nn.Module):
    ...
    def __init__(self, args, tranx: TransitionSystem):
        ...
        if args.decoder_cell == 'transformer':
            ...
            self.input_affine = nn.Linear(embed_size, args.decoder_hidden_size) if embed_size != args.decoder_hidden_size else lambda x: x
            self.decoder_network = Astormer(args.decoder_hidden_size, args.decoder_num_layers, args.num_heads, args.dropout)
        else:
            self.hidden2action = nn.Linear(args.decoder_hidden_size, embed_size) if args.decoder_hidden_size != embed_size else lambda x: x
            cell_constructor = ONLSTM if args.decoder_cell == 'onlstm' else LSTM
            self.decoder_network = cell_constructor(embed_size, args.decoder_hidden_size, num_layers=args.decoder_num_layers, dropout=args.dropout, dropconnect=args.dropout)
    ...
    def score(self, memories, batch):
        ...
        schema_embed = self.schema2action(schema_memory)
        ...
        if args.decoder_cell == 'transformer':
            ...
            for t in range(batch.ast_actions.size(1)): # need to extract parent hidden states (parent feeding)
                ...
                else: parent_state = encodings.new_zeros((len(batch), schema_embed.size(-1)))
    ...
    def parse(self, memories, batch, beam_size=5, n_best=5, decode_order='dfs+l2r'):
        ...
        if args.decoder_cell == 'transformer': prev_inputs = encodings.new_zeros((num_examples, 0, args.decoder_hidden_size))
        else:
                ...
                for bid, actions in zip(active_idx, prev_actions):
                    for action in actions:
                        if isinstance(action, ApplyRuleAction):
                            prev_action_embeds.append(self.production_embed.weight[action.token])
