...
"""
    中文数据集DuSQL评测脚本：计算预测 SQL 的 exact set match w/ and w/o values accuracy
    1. 对于select、where等子句的多个成分，如果仅是顺序不同也会算正确。
    2. 本脚本改自百度千言官方脚本，官方参考了Spider英文数据集中公开的evaluation.py代码(https://github.com/taoyds/spider)
    3. 解析的sql字典格式与nsts包中解析成的json格式不同（其需要转化为AST），评测时以此脚本为主，与官方评测指标一致
"""
...
def tokenize(string, single_equal=True):
    """
    Args:
    Returns:
    """
...
def get_sql(schema, query, single_equal=True):
    """
    Args:
    Returns:
    """
...
class Schema(object):
    ...
    def _map(self, schema):
        """map"""
        id_map = {'*': "__all__", 'time_now': "__time_now__"}
...
def evaluate_dusql(gold, predict, table, kmaps):
    """ Evaluate main function for dataset DuSQL.
    Args:
        gold (str): gold file name
        pred (str): predict file name
        table (str): all tables file name
        kmaps (dict): column set mappings
    Returns: exec -> exact set match with values
        dict {'exact': float, 'exec': float}
    """
    if type(table) == str:
    ...
    for ins_id, g in gold_dict.items():
        ...
        if exact_score_novalue == 0:
            print("Question id:", ins_id)
            print("{} pred EM: {}".format(hardness, pred_str))
            print("{} gold EM: {}".format(hardness, gold_str))
            print("")
        elif exact_score == 0:
            print("Question id:", ins_id)
            print("{} pred EMV: {}".format(hardness, pred_str))
            print("{} gold EMV: {}".format(hardness, gold_str))
            print("")
    ...
    print_scores(scores)
    return {'exec': scores['all']['exact'], 'exact': scores['all']['exact(no value)']}
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument('-t', '--table', dest='table', type=str, default='data/dusql/tables.json')
    args = arg_parser.parse_args()

    kmaps = build_foreign_key_map_from_json(args.table)
    results = evaluate_dusql(args.gold, args.pred, args.table, kmaps)
    print(results)